...
module userAssignedIdentity 'br/public:avm/res/managed-identity/user-assigned-identity:0.4.0' = {
  name: '${take(uniqueString(deployment().name, location),4)}-appGwUserAssignedIdentity'
...
module appGatewayAddCertificates 'app-gateway-cert.bicep' = if (!empty(base64Certificate) && !empty(applicationGatewayCertificateKeyName)) {
  name: '${take(uniqueString(deployment().name, location),4)}-appGatewayAddCertificates'
...
module applicationGatewayPublicIp 'br/public:avm/res/network/public-ip-address:0.7.1' = {
  name: '${take(uniqueString(deployment().name, location),4)}-applicationGatewayPublicIp'
  scope: resourceGroup(resourcesNames.resourceGroup)
  params: {
    location: location
...
module applicationGateway 'app-gateway.module.bicep' = {
  name: '${take(uniqueString(deployment().name, location),4)}-applicationGateway'
  scope: resourceGroup(resourcesNames.resourceGroup)
  params: {
    location: location
    tags: tags
    ...
    keyVaultCertUri: (!empty(base64Certificate) && !empty(applicationGatewayCertificateKeyName)) ? appGatewayAddCertificates!.outputs.SecretUri : ''
...
module appGwWafPolicy 'br/public:avm/res/network/application-gateway-web-application-firewall-policy:0.2.0' = {
  name: '${take(uniqueString(deployment().name, location),4)}-appGwWafPolicy'
  scope: resourceGroup(resourcesNames.resourceGroup)
  params: {
