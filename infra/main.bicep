...
metadata name = 'Container Apps Landing Zone'
...
@description('Optional. The size of the virtual machine to create. Required only when vmJumpboxOSType != "none".')
param vmSize string = 'Standard_B2s'

@description('Optional. The storage account type to use for the jump box. Defaults to `Standard_LRS`.')
param storageAccountType string = 'Standard_LRS'

@description('Optional. The password to use for the virtual machine. Required only when vmJumpboxOSType != "none" and vmAuthenticationType == "password".')
@secure()
...
@description('Optional. The base64 encoded certificate to use for Application Gateway certificate. If provided, applicationGatewayCertificateKeyName must also be specified.')
@secure()
param base64Certificate string = ''

@description('Optional. The name of the certificate key to use for Application Gateway certificate. Required when base64Certificate is provided.')
...
module spoke 'modules/networking/deploy.spoke-vnet.bicep' = {
...
module containerAppsEnvironment 'modules/container-apps/deploy.aca-environment.bicep' = {
  ...
  params: {
    ...
    acaUserAssignedIdentityId: supportingServices.outputs.acaUserAssignedIdentityId
    logAnalyticsWorkspaceId: spoke.outputs.logAnalyticsWorkspaceId
  }
}

module sampleApplication 'modules/container-apps/deploy.sample-application.bicep' = if (deploySampleApplication) {
  ...
  params: {
    ...
    userAssignedIdentityId: supportingServices.outputs.acaUserAssignedIdentityId
  }
}
...
output containerRegistryUserAssignedIdentityResourceId string = supportingServices.outputs.acaUserAssignedIdentityId
...
output applicationGatewayResourceId string = ''
...
output applicationGatewayPublicIp string = ''
